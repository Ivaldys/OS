...
void signal_handler(int signal) {
    ...
    for (int client_socket : clients) {
        ...
        write(client_socket, shutdown_msg.c_str(), shutdown_msg.length());
        close(client_socket);
    }
    clients.clear();

    // Notify all monitors to disconnect
    {
        std::lock_guard<std::mutex> lock(monitors_mutex);
        for (int monitor_socket : monitors) {
            std::string shutdown_msg = "SHUTDOWN";
            write(monitor_socket, shutdown_msg.c_str(), shutdown_msg.length());
